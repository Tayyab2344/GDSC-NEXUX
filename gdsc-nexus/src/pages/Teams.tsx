...
const Teams = () => {
  ...
  const { data: recruitmentForm } = useQuery({
    ...
    queryFn: async () => {
      // Check for 'recruitment' form (or any public form with this slug)
      const res = await fetch("http://localhost:3000/forms/public/recruitment");
      if (!res.ok) return null;
      // API returns the form object if found
      return res.json();
    },
    retry: false
  });
