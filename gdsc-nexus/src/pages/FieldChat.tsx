...
import { ArrowLeft, Send, Paperclip, Smile, MoreVertical, Users, FileText, Image, Trash2, Mic, X, Loader2 } from "lucide-react";
...
const FieldChat = () => {
  ...
  // Fetch Field Details (including Chat ID and Members)
  const { data: field, isLoading: isFieldLoading } = useQuery({
    queryKey: ["field", fieldId],
    queryFn: async () => {
      const res = await fetch(`http://localhost:3000/fields/${fieldId}`, {
        headers: { Authorization: `Bearer ${token}` }
      });
  ...
  // Cleanup realtime messages when switching chats
  useEffect(() => {
    setRealtimeMessages([]);
  }, [chatId]);
  ...
  const handleRemoveMember = async (userId: string) => {
    try {
      const res = await fetch(`http://localhost:3000/fields/${fieldId}/members/${userId}`, {
        method: "DELETE",
        headers: { Authorization: `Bearer ${token}` }
      });
    ...
    <div className="min-h-screen bg-background flex flex-col">
      ...
      <div className="flex-1 pt-24 flex container mx-auto px-4 pb-4 gap-4 h-[calc(100vh-1rem)]">
        ...
        <div className="w-72 border rounded-lg hidden lg:flex flex-col bg-card shadow-sm h-full">
                ...
                {field.members?.map((member: FieldMember) => {
                  ...
                  return (
                    <div key={member.userId} className="flex items-center justify-between group">
                      ...
                      {isAuthorizedLead && member.user.id !== currentUser?.id && (
                        <AlertDialog>
                          <AlertDialogTrigger asChild>
                            <Button
                              variant="ghost"
                              size="icon"
                              className="h-6 w-6 opacity-0 group-hover:opacity-100 text-google-red hover:text-google-red hover:bg-google-red/10 transition-opacity"
                              title="Remove member"
                            >
                          ...
                          <AlertDialogContent>
                            <AlertDialogHeader>
                              ...
                              <AlertDialogDescription>
                                Are you sure you want to remove {member.user.fullName} from this field? They will lose access to this chat.
                              </AlertDialogDescription>
                            </AlertDialogHeader>
                            <AlertDialogFooter>
          ...
          {/* Shared Files Section (Static for now, can be dynamic later) */}
