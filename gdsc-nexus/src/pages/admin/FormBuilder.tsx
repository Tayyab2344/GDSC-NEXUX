...
interface Form {
  ...
  slug: string;
  description: string;
  schema: FormSchema; // JSON in backend, parsed here
  isPublic: boolean;
  createdBy: string;
  createdAt: string;
  _count?: {
...
const FormBuilder = () => {
  ...
  // Local state for editing the active form before saving
  const [activeFormState, setActiveFormState] = useState<Partial<Form> | null>(null);

  // 1. Fetch User Profile (to check permissions)
  ...
  // 3. Fetch Single Form Details (active)
  const { data: activeFormData, isLoading: isLoadingActive } = useQuery({
    queryKey: ["form", selectedFormId],
    queryFn: async () => {
      if (!selectedFormId) return null;
      const res = await fetch(`http://localhost:3000/forms/${selectedFormId}`, {
        headers: { Authorization: `Bearer ${token}` }
      });
      if (!res.ok) throw new Error("Failed to fetch form details");
      const data = await res.json();
      // Ensure schema is parsed if it's a string (though Prisma usually handles JSON)
  ...
  // Sync activeFormState with fetched data
  useEffect(() => {
    if (activeFormData) {
      setActiveFormState(JSON.parse(JSON.stringify(activeFormData))); // Deep copy to edit
    }
  }, [activeFormData]);
  ...
  const fieldTypes = [
    ...
    // { icon: CheckSquare, label: "Checkbox", type: "checkbox" }, // Logic omitted for brevity
            ...
            <div className="lg:col-span-2">
              {activeFormState ? (
                    ...
                    <Card>
                      ...
                      <CardContent>
                        ...
                        <div className="space-y-3">
                          {(!activeFormState.schema?.fields || activeFormState.schema.fields.length === 0) && (
                            <div className="text-center p-8 border border-dashed rounded-lg text-muted-foreground text-sm">
                              No fields added yet. select a field type above.
                            </div>
                          )}
                          {activeFormState.schema?.fields.map((field) => (
                            <div
                              key={field.id}
                              className="flex items-center gap-3 p-4 rounded-lg border border-border hover:bg-muted/50 group"
                            >
                              ...
                              <div className="flex-1">
                                <div className="flex items-center gap-2">
                                  ...
                                  <Input
                                    value={field.label}
                                    onChange={(e) => {
                                      const newFields = activeFormState.schema!.fields.map(f =>
                                        f.id === field.id ? { ...f, label: e.target.value } : f
                                      );
                                      setActiveFormState({
                                        ...activeFormState,
                                        schema: { ...activeFormState.schema!, fields: newFields }
                                      });
                                    }}
                                    className="h-8 font-medium w-fit"
                                  />
                                ...
                                <span className="text-xs text-muted-foreground capitalize ml-1">{field.type}</span>
                              </div>
                              <Button
                                variant="ghost"
                                size="icon"
                                className="h-8 w-8 text-google-red"
                        ...
                        <div className="mt-6 flex gap-3">
                          <Button
                            variant="googleBlue"
                            onClick={() => updateFormMutation.mutate(activeFormState)}
                            disabled={updateFormMutation.isPending}
                          >
                            {updateFormMutation.isPending ? <Loader2 className="animate-spin w-4 h-4 mr-2" /> : <Save className="w-4 h-4 mr-2" />}
                            Save Form
                          </Button>
                          <Button variant="outline" onClick={() => window.open(`/forms/${activeFormState.slug}`, '_blank')}>
                ...
                <div className="h-full flex flex-col items-center justify-center border-2 border-dashed rounded-lg p-12 text-center text-muted-foreground">
                  ...
                  <h3 className="text-lg font-semibold">Select a form to edit</h3>
