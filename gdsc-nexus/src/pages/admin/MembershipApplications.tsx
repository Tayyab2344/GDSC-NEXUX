...
const MembershipApplications = () => {
    ...
    const [categoryFilter, setCategoryFilter] = useState("MEMBERSHIP");
    ...
    const isTechLead = !!leadField && !isManagement;
    ...
    const { data: applications, isLoading } = useQuery({
        ...
        queryFn: async () => {
            ...
            if (isManagement) {
                const res = await fetch(`${API_BASE_URL}/forms/type/${categoryFilter}/submissions`, {
                    headers: { Authorization: `Bearer ${token}` }
                });
            ...
            if (isTechLead && leadField) {
                const res = await fetch(`${API_BASE_URL}/forms/field/${leadField.name}/verified`, {
                    headers: { Authorization: `Bearer ${token}` }
                });
    ...
    const verifyMutation = useMutation({
        mutationFn: async (submissionId: string) => {
            const res = await fetch(`${API_BASE_URL}/forms/submissions/${submissionId}/verify`, {
                method: "POST",
                headers: { Authorization: `Bearer ${token}` }
            });
        ...
        onSuccess: () => {
            if (categoryFilter === 'MEMBERSHIP') {
                toast.success("Membership approved! User enroled in selected fields and awarded badge.");
            } else {
                toast.success("Application approved! Lead has been enroled and a password reset email has been sent.");
            }
    ...
    const approveMutation = useMutation({
        mutationFn: async ({ submissionId, fieldId }: { submissionId: string, fieldId: string }) => {
            const res = await fetch(`${API_BASE_URL}/forms/submissions/${submissionId}/approve`, {
                method: "POST",
                headers: {
    ...
    const updateStatusMutation = useMutation({
        mutationFn: async ({ id, status }: { id: string, status: string }) => {
            const res = await fetch(`${API_BASE_URL}/forms/submissions/${id}/status`, {
                method: "PUT",
                headers: {
                    "Content-Type": "application/json",
                    Authorization: `Bearer ${token}`
