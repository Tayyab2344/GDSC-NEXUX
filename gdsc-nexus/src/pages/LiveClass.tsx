...
const LiveClass = () => {
  ...
  const handleJoinClass = async () => {
    ...
    try {
      setIsJoining(true);
      // Mark attendance
      const res = await fetch(`http://localhost:3000/classes/${classId}/join`, {
        method: "POST",
        headers: { Authorization: `Bearer ${token}` }
      });
      ...
      if (res.ok) {
        ...
        // We still allow joining even if attendance fails, but warn user? 
        // Or just fail silently on attendance part to not block learning.
      }
